...
pyglet.font.add_file(os.path.join(BASE_DIR, 'fonts', 'league-spartan.bold.ttf'))
pyglet.font.add_file(os.path.join(BASE_DIR, 'fonts', 'GlacialIndifference-Regular.otf'))
...
class GameFrame(tk.Frame):
    def __init__(self, master):
        ...
        # Labels and Buttons
        self.hint_clicked_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'hint_bg.png')).subsample(20, 20)
        self.hint_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons','hint_no_bg.png')).subsample(20, 20)
        self.back = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'back.png')).subsample(5, 5)
    ...
    def save_grid_state(self, filename='grid_state.json'):
        """
    ...
    def view_complete(self, level):
        """
        Displays the complete puzzle image for the given level.
        """
        ...
        base_path = os.path.join(os.path.dirname(__file__) , "assets", "CorrectPuzzle")
        image_file = os.path.join(base_path, f"Level{level}.png")

        try:
            # Load the image dynamically based on the level
            self.level = Image.open(image_file)
            ...
            self.hint_back = tk.Button(
                ...
                command=self.exit_complete_puzzle
            )
            self.hint_back.pack(side='bottom', pady=2)

        except FileNotFoundError:
            print(f"Error: File not found for level {level}. Ensure the file exists at {image_file}.")
        except Exception as e:
...
class SelectLevel(tk.Frame):
    ...
    def __init__(self, master):
        ...
        # Level buttons
        self.one = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'one.png')).subsample(3, 3)
        self.two = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'two.png')).subsample(3, 3)
        self.three = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'three.png')).subsample(3, 3)
        self.four = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'four.png')).subsample(int(2.8), int(2.8))
        self.five = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'five.png')).subsample(3, 3)
        self.six = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'six.png')).subsample(3, 3)
        self.seven = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'seven.png')).subsample(3, 3)
        self.eight = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'eight.png')).subsample(3, 3)
        self.back = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'back.png')).subsample(3, 3)
...
class MainMenu(tk.Frame):
    def __init__(self, master):
        ...
        self.info_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'info.png')).subsample(18,18)
        self.play_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'play.png')).subsample(2, 2)
        self.exit_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'exit.png')).subsample(2, 2)
        self.sound_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'sound.png')).subsample(5, 5)
        self.sound_off_image = tk.PhotoImage(file=os.path.join(BASE_DIR, 'buttons', 'sound_off.png')).subsample(5, 5)
        ...
        self.info_button.place(relx = 0.095, rely = 0.86)

        # Bind hover effects
        self.info_button.bind('<Enter>', lambda e: self.on_hover(self.info_button))
        self.info_button.bind('<Leave>', lambda e: self.on_leave(self.info_button))
    ...
    def show_info(self):
        messagebox.showinfo("Game Information", "ColorMatch Hue Puzzle is a game developed by Rolf Genree L. Garces as his final project for CMSC 122: Data Structures and Algorithm I. "
                                                "This is inspired by Color Puzzle and all the level configuration are taken from the game."
                                                "This game features dynamic grid challenges with varied grid sizes and randomized tile "
                                                "placements at each level. Each level also includes some immovable, correctly placed tiles "
                                                "to guide players. Unlike similar games, ColorMatch Hue Puzzle offers a hint system with optional, "
                                                ...
                                                " Should you be interested in the data structures and algorithm used, you may read the summary of presentation"
                                                "in the pdf file, 'ProjectDetails'.")
    def play_sound(self):
        if self.sound_on:
            try:
                mixer.music.load(os.path.join(BASE_DIR, 'music', 'bg_music.mp3'))
...
class ColorMatch(tk.Tk):
    def __init__(self):
        ...
        img_path = os.path.join(BASE_DIR, 'logo', 'Logo.png')
